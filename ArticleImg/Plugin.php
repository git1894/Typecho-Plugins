...
class ArticleImg_Plugin implements Typecho_Plugin_Interface
{
	...
	public static function setThumbnail($post) {
		$db = Typecho_Db::get();
		$row = $db->fetchRow($db->select('thumbnail')->from('table.contents')->where('cid = ?', $post->cid));
		if($row){
			$html = '
			<section class="typecho-post-option">
				<label for="thumbnail-url" class="typecho-label">文章顶部图片URL</label>
				<p><input id="thumbnail-url" name="thumbnail-url" type="text" value="'.$row['thumbnail'].'" class="w-100 text" /></p>
			</section>
			';
			_e($html);		  
		}else{
			...
			<section class="typecho-post-option">
				<label for="thumbnail-url" class="typecho-label">文章顶部图片URL</label>
				<p><input id="thumbnail-url" name="thumbnail-url" type="text" value="" class="w-100 text" /></p>
			</section>
			';
			_e($html);				
		}
	...
	public static function deactivate(){
	  ...
	  if($delFields){
		  $db = Typecho_Db::get();
		  $prefix = $db->getPrefix();
		  $db->query('ALTER TABLE `'. $prefix .'contents` DROP `thumbnail`;');
	  }
	}
	...
	public static function sqlInstall()
	{
	  ...
	  $prefix = $db->getPrefix();
	  try {
		$select = $db->select('table.contents.thumbnail')->from('table.contents');
		...
		if(('Mysql' == $type && (1054 == $code || '42S22' == $code)) ||
		  ...
		  try {
			if ('Mysql' == $type) {
			  $db->query("ALTER TABLE `".$prefix."contents` ADD `thumbnail` VARCHAR(255) NOT NULL  DEFAULT 'unknown' COMMENT '文章顶部缩略图URL';");
			} else if ('SQLite' == $type) {
			  $db->query("ALTER TABLE `".$prefix."contents` ADD `thumbnail` VARCHAR(255) NOT NULL  DEFAULT 'unknown'");
			} else {
	...
	public static function changeURL($contents, $post)
	{
	  ...
	  $thumburl = $post->request->get('thumbnail-url', _t($defaultUrl));
  		$db = Typecho_Db::get();
	  $sql = $db->update('table.contents')->rows(array('thumbnail' => $thumburl))->where('cid = ?', $post->cid);
  		$db->query($sql);
	}
	/**
	 * 处理默认图片
	...
	public static function render($value, $archive) {
		if (!(isset($value['thumbnail']))) return $value;
		if ($value['thumbnail'] === "unknown") {
			$value['thumbnail'] = Typecho_Widget::widget('Widget_Options')->plugin(str_replace("_Plugin","",__CLASS__))->defaultUrl;
			if (empty($value['thumbnail'])) {
				preg_match_all( "/<[img|IMG].*?src=[\'|\"](.*?)[\'|\"].*?[\/]?>/", $archive->content, $matches );
				if(isset($matches[1][0])){
					$value['thumbnail'] = $matches[1][0];
				}
			}
			if (empty($value['thumbnail'])) $value['thumbnail'] = Typecho_Widget::widget('Widget_Options')->siteUrl . str_replace("/usr/","usr/",__TYPECHO_PLUGIN_DIR__). '/ArticleImg/rand/' . mt_rand(0, 9) . '.jpg';
		}
		return $value;
	}
	/**
...
span.set-thumbnail:hover {
	text-decoration:underline
}
</style>
<script>
	$(document).ready(function() {
		link = '<span class="set-thumbnail">设置为题图</span>'
		$('#file-list').hover(function() {
			list = $('li',$('#file-list'));
			for(i=0;i<list.length;i++) {
				infoPanel = $('.info', list[i]);
				if ($('.set-thumbnail',infoPanel).length == 0) {
					infoPanel.append(link);
					$('.set-thumbnail',infoPanel).click(function() {
